##############################################################################
# model from https://github.com/Yuanbo2020/cSEM
##############################################################################
...
class CnnT(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.conv_block1(x, pool_size=(4, 2), pool_type='max')
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.conv_block2(x, pool_size=(4, 2), pool_type='max')
        x = self.bn2(x)
        x = self.relu(x)
        if self.dropout:
            x = F.dropout(x, p=0.2, training=self.training)
        ...
        ############################# scene ################################################################
        x_scene = self.conv_block3(x_com, pool_size=(4, 2), pool_type='max')
        x_scene = self.bn3(x_scene)
        x_scene = self.relu(x_scene)
...
if __name__ == '__main__':
    model = CnnT(classes_num=2, batchnormal=True, dropout=True)
    print(model)
    input = torch.randn(64, 1500, 64)
    output = model(input)
    print(output.size())
